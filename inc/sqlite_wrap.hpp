...
namespace tetra { namespace sqlite
{
	struct ScopeExit
	{
	...
	public:
		 ...
		 * satement processing as the statement only needs to be parsed once.
		 **/
		SafeStmt PrepareStatement( const std::string& stmt );

		/**
		 * Executes the given functor on the statement until there are no more
		...
		/**
		 * Executes the given sql statement a single time and resets its virtual
